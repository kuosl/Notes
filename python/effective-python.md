...
1. 内置的concurrent.future模块利用另外一个名叫multiprocessing的内置模块，以子进程的形式平行地运行多个解释器，从而令python能够利用多核cpu来提升速度。    
...
* 常规方法是：把with后面的值定义新类的对象，为类定义特殊方法：**enter**和**exit**
